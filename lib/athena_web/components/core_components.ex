defmodule AthenaWeb.CoreComponents do
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="mt-10 bg-transparent space-y-8">
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center text-sm text-gray-100 gap-4 leading-6">
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold text-gray-100 leading-6">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-gray-50">
          <%= render_slot(@inner_block) %>
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-sm text-gray-100 leading-6">
